...
The SAX transformation procedure described in the previous section yields symbolic time-series based on the real-valued data. Having such a symbolic representation (also called in the literature as a \textit{symbolic temporal data}) is very advantageous comparing to the real-valued data due to the vast amount of the algorithms developed for the string processing. Moreover, having homogenious symbolic series corresponding to the low-level process artifacts combined with the symbolic event streams from the SCM or bug tracking system potentially 
