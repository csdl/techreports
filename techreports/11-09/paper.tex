...
\title{Recognizing development behaviours corresponding to Android OS evolution and release life-cycle}
...
\IEEEauthorblockA{CSDL laboratory, ICS Department\\
...
succesfully partinioned by the time of the day reflecting our biological nature 
\cite{citeulike:10392325} \cite{citeulike:10392305}. However external constraints, 
such as employement constraints, software release cycle \cite{citeulike:2739216} 
and management constraints \cite{citeulike:6095797} found to be able to significantly alter 
natural activity patterns. Furthermore, within the open-source projects whith 
...
Essentially these trails are event-series where every event is timestamped and enclose 
...
polynomial, wevelets, piecewise, symbolic, etc. However, in spite of the fact that there are
...
tree, author and a commiter identification, change message and affected targets. 
...
\section{Results}
\subsection{Kernel-OMAP life cycle patterns dicovery}
I arbitrary selected the Android kernel-OMAP as one of the large Android OS subprojects. 
...
with $@android.com$ e-mail domain and a telemetry stream of $ADDED_LINES$.
...
While hierarchical clustering is a good sanity test, the K-means clustering is the most commonly used 
data mining clustering algorithm \cite{citeulike:3562}, so for completeness of the test I performed
k-means on the symbolic representation of data using $TF\ast IDF$ similarity. Algorithm converged after three
iterations.
...
As a training set for building dictionaries of pre and post-release patterns I choose \textit{Android 1.0},
\textit{Android 1.5} and \textit{Android 2.0} releases. For every release in the set I have selected four 
weeks before the release as \textit{pre-release} and four weeks after release as \textit{post-release} 
training intervals. The results 

\begin{figure}[htb]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/omap-google-added.ps}
  \caption{Hierarchical clustering of pre and post commit patterns}
  \label{fig:kernel_changes}
\end{figure}
