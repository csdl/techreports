...
%\title{Grammar compression-driven anomaly discovery\\ in time series}
...
%\textbf{(Arnold) The above paragraph needs to be updated with the new section organization.(Pavel) updated.}
...
% \textbf{Crystal: may want to elaborate on this definition as per Reviewer's comment}
...
%\textbf{Crystal: may want to clarify S1,1:3 notation; maybe write out explicitly like S1,1:3 = aac1 aac2 abc3, which has a SAX word length of 3}
...
%\textbf{(Jessica) Since we referred to Figure 3 as well 2 paragraphs back, we should add a paragraph explaining Figure 3: the dataset and the interpretation of results.}
...
%The  subsequences  corresponding to not included into rules intervals added to the top  of the $Outer$ list. \textbf{(Jessica) This last sentence sounds weird. Would you rephrase?} These are our best candidates for anomalies. 
...
\end{equation}
%\vspace{0.2em}
%\textbf{Pavel: Maybe there is a similar distance that has been defined in some previous work?}
...
%\textbf{Crystal: can the wording in the caption for Fig 5 be clarified? '...due to the larger value of normalized by the subsequence length Euclidean distance...'. Also, not sure if we want to rework Table 1, Reviewer 3's comment 7 showed confusion with the 'HOTSAX and RRA discords length and overlap' column}
...
%\textbf{(Arnold) Pavel, it would be good if you can add some figures (different from the already existing ones) and explanations that show cases where our techniques did well e.g., short anomalies, and where it failed. You mention that that "careful parameter selection" was needed, it would nice if you can elaborate on some of the heuristics that was used to select the parameters. Ideally, comprehensive figures relating to sensitivivity analysis would be good, but as we've discussed there are some issues with that. Perhaps you can include some choice results that are not too contestable, that would show that we did our due diligence in our experimentation eval.}
...
%\textbf{(Pavel) I think we may need to link this work with the previous effort \cite{msequitur}, so it is not perceived out of the context. (Arnold) Since we've already mentioned the spatial data eval in the beginning, this subsection appears to be within context.}
...
%\textbf{Crystal: please clarify this last sentence; how are long-term correlations related to discovering accurate anomalies? (Pavel) I've just rewritten that.}
%, which makes it difficult to capture long-term correlations
...
% The sequence of SFC visit order along with their times was passed to the RRA algorithm for processing.
%\textbf{Crystal: the Figure number is not showing up correctly here, I think it's missing the proper Latex "ref" tag. Also, maybe the last sentence can be altered to be like, "The Hilbert SFC transformed trajectory produces a time series, which is then passed to the RRA algorithm for processing."}
...
%\textbf{(Jessica) One potential problem is that we have not reported any of the SAX parameters used --- this makes the experiments non-reproducible. I don't know if the database community is as picky as the data mining community on this, but we might get dinged by the reviewers for lack of parameter details. Do you happen to have all the parameter information recorded? Or can we make any general statement like what I did in earlier papers, e.g. alphabet size 3~6 is good, winsize is data dependent (perhaps emphasize the advantage of having the interactive visualization tool here)? Or is the algorithm so sensitive to the parameters that we cannot make such a statement? To the least, perhaps we should have a subsection on Parameter Setting, and move the "lessons learned" from conclusion to this subsection?}
...
%\section{Algorithmically anomalous\\ subsequence utility}\label{utility}
%\textbf{(Arnold) Also consider having this section be converted to a subsection of the experimental eval. section as a way to summarize key points and results.}
...
%\textbf{Crystal: in the caption for Fig 10, can the word "video" be removed?}
...
%We proposed a novel, variable-length time series anomaly discovery technique that is based on grammatical compression. As a grammar induction algorithm learns regularities and builds a compact grammar from the input string obtained via sliding window-based time series symbolic discretization, it fails to compress subsequences that are irregular to the rest of the string. We consider these irregular subsequences to be anomalous. We explained our approach efficiency and effectiveness using the general notions of Kolmogorov complexity and algorithmic randomness. In addition, through an experimental evaluation, we have shown that the proposed technique is exceeding the current state of the art in efficiency and resolution.
