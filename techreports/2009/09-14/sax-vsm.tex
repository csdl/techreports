...
%NOREDUCTION,36,5,5,0.5666666666666667,0.43333333333333335
%EXACT,36,5,5,0.5333333333333333,0.4666666666666667
%CLASSIC,36,5,5,0.4666666666666667,0.5333333333333333
%NOREDUCTION,19,17,3,0.7666666666666667,0.23333333333333328
%EXACT,19,17,3,0.8,0.19999999999999996
%CLASSIC,19,17,3,0.6333333333333333,0.3666666666666667
%NOREDUCTION,87,6,5,0.6,0.4
%EXACT,87,6,5,0.6666666666666666,0.33333333333333337
%CLASSIC,87,6,5,0.6,0.4
...
through UCR repository \cite{ucr}. These datasets represent a variety of data types that are usual to TSC problems.
...
Train data were used in cross-validation for optimization of SAX parameters using \mbox{DIRECT}. 
Once found, the optimal parameter settings were used to assess SAX-VSM classification accuracy on 
the test data. The last column of table Tables \ref{perf_table1} and \ref{perf_table2} reports the 
SAX-VSM classification accuracy.
...
As per the running time cost, to no surprise, the DTW-based classifier was found as the most expensive technique. 
...
The similar behavior was observed in experiments with Two Patterns dataset. The Figure \ref{fig:venn-2p} shows
...
can be prohibitive to the approach applicability. 

In contrast, as shown above, SAX-VSM extracts and weights all patterns at once, without any added cost. 
...
and show a number of its application to a real-life datasets.
...
\begin{figure}[!ht]
   ...
   \caption{An example of the heatmap-like visualization that exploits the subsequence ranking in order to highlight 
   time-series fragments that are highly characteristic to the class.
   ...
   If a subsequence was found to be characteristic to other than the time series class its weight has been subtracted, 
   if it was from the same class, the weight has been added. Highlighted by the visualization features corresponding to a 
   ...
   patterns are local and ordered.}
   \label{fig:heat2}
\end{figure}
...
By following previously mentioned shapelet-based work \cite{citeulike:7344347} \cite{citeulike:11345338}, 
I have used a well-studied \textit{GunPoint} data set \cite{Ratanamahatana04makingtime-series} to explore the 
...
The time series of this problem are obtained by transforming the instant coffee spectroscopy data with diffuse reflection infrared 
Fourier transform (DRIFT) and truncating them to 286 data points from the region 800-1900 cm$^{-1}$. 

The two top-ranked by SAX-VSM subsequences in both datasets correpond to spectrogram intervals of 
