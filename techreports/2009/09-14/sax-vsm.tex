...
A \textbf{\textit{Time Series}} time series is a sequence $T=(t_{1},t_{2},...,t_{m})$ that is an ordered set of $m$ real values.
\end{defn}
While the ordering is typically temporal, ordering in other kinds of data such as shapes \cite{citeulike:4384496} and spectrograms 
\cite{citeulike:12550833} also can be well defined to be considered as “time series” for the purpose of indexing and mining. 
Without loss of generality in the research literature the equispaced data is typically considered, which is not true
for mining of software process artifact trails where time spacing varies between successive artifacts. 
However, for simplicity and without loss of generality, in this chapter I consider equispaced time series in this chapter.
...
series which are obtained from overlapping subsequences. Usually, overlapping subsequences are extracted with the 
...
Previously, the analysis of the results showed that the best matches for an extracted by sliding window subsequence 
tend to be its immediate neighbors, in particular the subsequence one point to the right and the subsequence
one point to the left \cite{citeulike:3977965} \cite{citeulike:3175749}. 
The authors defined these matching subsequences as the \textit{trivial matches}.
They found, that in the smooth regions of the time series, the amount of trivial matches can be large and enough that 
they would dominate over true patterns in any sliding-window technique due to over-counting. 
This would not only distorts the results, but could make them meaningless \cite{citeulike:227029}. 
Therefore, when extracting subsequences from the time series by a 
sliding window, the trivial matches should be excluded. 

The authors proposed a sampling strategy based on a \textit{MINDIST} \ref{eq:sax_mindist} designed in the order to avoid 
``trivial and degenerative solutions''. 	If $l$ consecutive SAX words 
...
retrieve all occurrences of $C_{i,k}$, it will be possible to find its first occurrence and slide to the right until the 
word which is different from $C_{i,k}$. While the authors found the inclusion of the numerosity reduction 
vital for motif and discords discovery, intuitively, in the case of SAX-VSM, the over-counting effect is significantly 
mediated by design of \textit{tf$\ast$idf} statistics \eqref{formula:tfidf}. 
Moreover, the exclusion of ``trivial matches'' may degrade the overall performance of bag of words based 
approach, as pointed in \cite{citeulike:10525778}.

In order to clarify this issue, I have conducted a series of experiments for three sampling strategies \ref{nr_study} 
and found, that for most of the data sets, the inclusion of the numerosity reduction significantly reduced DIRECT 
scheme convergence time and improved its accuracy. 
Furthermore, once I have relaxed the ``triviality constraints'' by substitution of \textit{MINDIST} function \ref{eq:sax_mindist}
...
this numerosity reduction strategies in detail.

As noted in \cite{citeulike:2821475}, given a character subsequence $\widehat{S}_{i}$ extracted by the use 
of sliding window and SAX, it is very likely, that $\widehat{S}_{i}$ is very similar to its
...
is in the smooth region of the time series - due to the smoothing effect of PAA \ref{section-sax}. 
...
\label{eq:hamming}
\end{equation} 
where $ascii\_dist( \widehat{q_{i}}, \widehat{c_{i}} )$ is the absolute value of the difference between ASCII codes for 
letters $\widehat{q_{i}}$ and $\widehat{c_{i}}$.
...
\begin{itemize}
  ...
  \item \textit{corpus} - a set of bags, and 
...
\begin{table}
\caption{The SMART notation \cite{citeulike:4469058}. }
\vspace{0.4cm}
\label{tbl:smart}
...
%which consists of the conversion of an unlabeled time series into a terms \\[10ex]
