...
Two promising alternatives combining strengths from the both categories were recently proposed.
...
%SAX-VSM is based on two techniques. 
%The first technique is Symbolic Aggregate approXimation, which is a high-level symbolic representation of time series \cite{sax} 
%that is used in order to transforms real-valued time series into bags of SAX words. 
%The second technique is Vector Space Model based on \tfidf weighting scheme \cite{citeulike:300428}, that is used for SAX words 
%ranking. The classification relies on Cosine similarity.
...
SAX also introduces a new metric for measuring the distance between strings by extending Euclidean and PAA \cite{citeulike:2946589} distances. 
...
As shown by Lin et al. \cite{sax}, this SAX distance metrics lower-bounds the PAA distance, i.e.
\begin{equation}
...
In particular, two different patterns are used to define classes: an upward step and a downward step as 
...
For small training data sets, SAX-VSM was found to be significantly more accurate than 1NN classifiers. 
However, by the time there were more than 500 time series in a training set, there was no statistically 
significant difference in accuracy (Fig. \ref{fig:precision-runtime}, left). 
...
This particular feature -- the ability to discover class-characteristic subsequences -- is exploited
by Software Trajectory Analysis which reports them to the user for further evaluation.
