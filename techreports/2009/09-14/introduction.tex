...
SAX-VSM represents a significant contribution to the field of time series classification and 
addresses two core problems in the field: the characteristic feature selection and the classification results interpretation. 
...
for SAX-VSM-based time series classification. The approach is based on the DIRECT optimization scheme and 
converges to the optimal parameters set by orders of magnitude faster than a na\"ive grid-based search.

\item \textbf{Novel time series class-characteristic subsequence heatmap-like visualization}
SAX-VSM ranks all extracted via sliding window time series subsequences. 
...
class-characteristic (descriptive) power.

\item \textbf{SAX-VSM implementation and empirical evaluation}
SAX-VSM was implemented and open-sourced \cite{jmotif}. I developed SAX-VSM over a period of five years,
...
patterns providing the superior results interpretability and meaningful visualization.

\item \textbf{STA reference implementation and evaluation}
Software Trajectory Analysis framework was implemented and open-sourced \cite{sta-source}. 
...
the results of use case-based empirical evaluation of STA capacity to discover useful recurrent behaviors including: 
\begin{enumerate}
