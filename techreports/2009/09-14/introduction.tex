...
of recurrent behaviors through the analysis of software process artifacts \cite{csdl2-10-09} \cite{csdl2-11-10}. 
STA represents a significant contribution to the field of Software Engineering where it can be 
applied to problems of software process discovery, software process elicitation, and mining of software repositories.
...
time series classification and addresses two core problems in the field: 
...
I this work I present an approach for SAX parameters optimization which guarantees to finds the optimal parameters set
for SAX-VSM-based time series classification. This general solution is a significant contribution to the field of time series
analysis as it can be adopted for many other SAX-based algorithms.
The approach is based on the DIRECT optimization scheme \cite{citeulike:12563460} 
and cross validation. The proposed optimization process converges to the optimal parameters set by orders of 
magnitude faster than a na\"ive grid-based search \cite{sax-vsm}.

\item \textbf{Novel time series class-characteristic subsequence heatmap-like visualization}
SAX-VSM ranks all extracted via sliding window time series subsequences by class-characteristic power. 
By combining the rank coefficients of all subsequences that span a time series point, it is possible to color each point 
according to its class-characteristic specificity. 
This visualization produces a heat map-like time series plot that instantly highlights its segments according to their 
class-characteristic power \cite{sax-vsm}.
...
contribution to the fields of time series classification and mining.
...
At the same time, I shown that SAX-VSM capable to efficiently and effectively discover and rank class-characteristic 
...
Software Trajectory Analysis framework was also implemented and open-sourced \cite{sta-source}. 
I developed STA over a period of five years and the project is currently consist of over 40,000 lines of source code.

In order to assess the STA performance I conducted an empirical evaluation and present in this dissertation
the results of use case-based empirical evaluation of STA capacity to discover process-characteristic recurrent 
behaviors including: 
\begin{enumerate}
 \item the software release-related recurrent behaviors from Android OS and PostgreSQL software development processes;
 \item the ``Commit Fest''-related recurrent behaviors from PostgreSQL software development process; 
...
field of Software Engineering and to Mining Software Repositories in particular as they not only indicate
the feasibility of recurrent behaviors discovery, but allow to jump-start a new research project.
