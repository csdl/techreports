...
Then, I have outlined the differences of FLOSS processes from traditional Software Engineering 
...
(Section \ref{section_public_repositories}) that is worth to explore for a better understanding of FLOSS 
software processes. These provide a motivation to my exploratory study, whose details I outline in this section.

In my work, I attempt to explore a possibility of discovery of a specific human-driven aspect in software 
development that is a \textit{\textbf{behavior}}, which I define as the mannerism in which a developer, 
or a team, conduct their everyday work. 
In particular, I explore the possibility of discovery of \textit{recurrent behaviors} ( i.e. supported by the 
numerous evidence) from software process artifacts. 

To begin with, I hypothesized, that \textbf{it is possible to discover recurrent behaviors from publicly 
...
attention to some particular part of the code, or his discipline. Moreover, by using the simple information from 
commits - churn metrics for code and tests - it would be possible to see if test changes co-occur with code
changes.
Consider another example - one of the developers usually commits code changes in early mornings while 
another only at late in the day. For whatever reason it happens, these recurrent behaviors are also relatively 
easy to quantify, moreover they might indicate a number of constraints that are put on the project, process, 
or on the developers themselves.
...
extend our knowledge of software processes, but will lay a foundation for future research in software 
processes.
...
\cite{csdl2-10-09}. Similarly to other studies, I discovered, that while FLOSS process artifacts are 
numerous and easily accessible, their irregular, snapshot-like nature and the poor informational 
content significantly limit the applicability of known techniques for process mining.

In order to overcome this issue, I casted the initial problem of event-based recurrent behaviors 
discovery into more generic problem of knowledge discovery from time series that are constructed 
from process artifacts-extracted software metrics. To solve this, I proposed a novel technique 
for software process discovery 
