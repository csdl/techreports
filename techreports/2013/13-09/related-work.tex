...
\begin{figure}[ht!]
	\centering
		\includegraphics[width=0.65\columnwidth]{oberlin-design}
...
\begin{table}[ht!]
  \centering
        ...
        \hline
      \tabhead{School} & \tabhead{Feedback} & \tabhead{Education/ Activities} & \tabhead{Points} & \tabhead{Social} & \tabhead{Infrastructure} \\
                \hline
                Harvard University 	& manual 		& 	&  		&  	& simple webpage\\
                Wellesley College  	& manual 		& 	&  		&  	& simple webpage\\
                Duke University     	& manual 		& \multicolumn{1}{|c|}{\checkmark} & \checkmark &   & custom website \\
Western Washington University & manual 		& \multicolumn{1}{|c|}{\checkmark} & \checkmark &   & custom website \\
                Oberlin College 	    	& real-time 	& \multicolumn{1}{|c|}{\checkmark} &  	& \checkmark  & custom website \\
                Bowdoin College 	& real-time 	& \multicolumn{1}{|c|}{\checkmark} &  	& \checkmark  & out-source \\
   University of British Columbia & real-time      & \multicolumn{1}{|c|}{\checkmark} &  	& \checkmark  & out-source \\
Campus Conservation Nationals  & real-time   & \multicolumn{1}{|c|}{\checkmark} &  	& \checkmark  & out-source \\
                \hline
        \end{tabular}
...
\begin{table}[ht!]
  \centering
        ...
        \hline
      \tabhead{Game} & \tabhead{Type} & \tabhead{Targeted player} & \tabhead{Rewards} & \tabhead{Competition} & \tabhead{Mobile} \\
        \hline
...
\begin{figure}[ht!]
	\centering
		\subfigure[Satisfies Human Needs (source: Bunchball\cite{bunchball})]{\label{fig:human-needs}\includegraphics[width=4.2in]{human-needs.eps}}
		\subfigure[Basic Mechanics (source: Deterding \cite{Deterding2011meaningful})]{\label{fig:basic-elements (source: }\includegraphics[width=4.2in,height=2.4in]{basic-element.eps}}
...
\begin{figure}[ht!]
	\centering
		\subfigure[Player Journey]{\label{fig:player-lifecycle}\includegraphics[width=0.49\linewidth]{kim-workshop.eps}}
		\subfigure[Game Design]{\label{fig:game-design}\includegraphics[width=0.49\linewidth]{kim-workshop2.eps}}
...
\begin{table}[ht!]
  \centering
        ...
        \hline
      \tabhead{Mechanics} & \tabhead{Description} & \tabhead{Player Type} & \tabhead{In Makahiki}  \\
        \hline
...
\begin{table}[ht!]
  \centering
 	...
 	Points		&  & \checkmark \\
        Integrated Educational Content &  & \checkmark \\
 	Pluggable games &  & \checkmark \\
 	Personalization &   & \checkmark \\
        Game analytics &   & \checkmark \\
...
A game framework is used to create serious games. \autoref{fig:serious-game-lifecycle} illustrates a full lifecycle of a serious game using a framework. By instantiating from a serious game framework, the system admins create a new serious game with the default configurations. The next phrase is for the game designers to configure the instance according to their organization's specific needs. In the case of the built-in features not able to meet the organization's need, the game can be extended by the game developers to add new features or customizations. After the game instance is finalized, the system admins will deploy it to the infrastructure of the choice of the organization. Once the game instance is deployed and running, the next phrase is for the game managers to manage the game using the admin interface provided by the framework, and the players will play the game. The last phrase of the lifecycle is the analysis phrase after the game is over where game managers, designers and developers can evaluate and learn from the results of the game.

\begin{figure}[ht!]
	\centering
		\includegraphics[width=0.8\columnwidth]{serious-game-lifecycle.eps}
		\caption{Serious game lifecycle using a framework}
		\label{fig:serious-game-lifecycle}
\end{figure}

The design of Makahiki follows closely the serious game lifecycle using a framework, which provides building blocks and utilities for each phrase of the lifecycle. Lucid's Building Dashboard system only provides a pre-built Software As a Service (SaaS) solution where the game designers can choose from a small sets of dashboard widgets and game managers can enter energy data manually if needed. There is no option for system admin to install the system locally or deploy to the infrastructure of their choices, or for the developers to extend the game. There is no game analytics provided in BuildingDashboard. Makahiki has a much richer feature set than Lucid's Building Dashboard. 
...
De Freitas and Oliver \cite{de2006can} point out that there are few frameworks to support the evaluation of educational games. They introduce a four dimensional framework for evaluating 
...
\begin{figure}[ht!]
	\centering
		\subfigure[Triadic Game Evaluation (TGE) \cite{harteveld2010triadic}]{\label{fig:triadic-game-eval}\includegraphics[height=2.1in]{triadic-eval.eps}}
		\subfigure[Four Dimensional Framework for Evaluating Educational Games \cite{de2006can}]{\label{fig:four-dimensional-framework}\includegraphics[height=2.1in]{game-eval.eps}}
...
The above assessment methods focus on one or two aspects of the lifecycle of a serious game using a framework. \autoref{fig:lifecycle-assessment} illustrates the existing assessment methods in the context of a full serious game lifecycle using a serious game framework. 
\begin{figure}[ht!]
	\centering
		\includegraphics[width=0.8\columnwidth]{assessment-lifecycle.eps}
		\caption{Existing assessment methods in a serious game lifecycle}
		\label{fig:lifecycle-assessment}
\end{figure}

The serious game assessment methods only assess the player aspect of the lifecycle, where the player effectiveness is the main concern of the individual serious game assessments. The Game Engagement Questionnaire (GEQ) only assesses the engagement from the player's perspective. The Technology Acceptance Model (TAM) focuses on the perceived acceptance of the framework in the case of system admins and game designers' perspective. The Questionnaire for User Interaction Satisfaction (QUIS) only assesses the usability of players and game designers when using the game and the framework.

The design of SGSEAM, the Serious Game Stakeholder Experience Assessment Method, is to address all the stakeholders in a serious game lifecycle using a serious game framework. As illustrated in \autoref{fig:sgseam-assessment}, SGSEAM assess all the stakeholders in the full lifecycle of a serious game to understand the strengths and weaknesses of a serious game framework from these stakeholders' perspectives.

\begin{figure}[ht!]
	\centering
		\includegraphics[width=0.8\columnwidth]{sgseam-lifecycle.eps}
		\caption{Existing assessment methods in a serious game lifecycle}
		\label{fig:sgseam-assessment}
\end{figure}
