...
A game framework is used to create serious games. \autoref{fig:serious-game-lifecycle} illustrates the lifecycle of a serious game using the Makahiki framework. During the instantiation phase, the system admins create a new serious game with default configurations. The next phase requires the game designers to configure the instance according to their organization's specific needs. In the case that the built-in features are not able to meet the organization's need, the game can be extended by the game developers to add new features or customizations. After the game instance is finalized, the system admins deploy it to the infrastructure of choice for the organization. Once the game instance is deployed and running, the next phase requires the game managers to manage the game using the admin interface provided by the framework, and the players will play the game. The last phase of the lifecycle is the analysis phase after the game is over where game managers, designers and developers evaluate and learn from the results of the game.
...
The design of Makahiki follows closely the serious game lifecycle using a framework, which provides building blocks and utilities for each phase of the lifecycle. Lucid's Building Dashboard system only provides a pre-built Software As a Service (SaaS) solution where the game designers can choose from a small sets of dashboard widgets and game managers can enter energy data manually if needed. There is no option for system admin to install the system locally or deploy to the infrastructure of their choices, or for the developers to extend the game. There is no game analytics provided in BuildingDashboard. Makahiki has a much richer feature set than Lucid's Building Dashboard. 
...
The design of SGSEAM, the Serious Game Stakeholder Experience Assessment Method, is intended to address all the stakeholders in a serious game lifecycle using a serious game framework. As illustrated in \autoref{fig:sgseam-assessment}, SGSEAM assesses all the stakeholders in the full lifecycle of a serious game to understand the strengths and weaknesses of a serious game framework from these stakeholders' perspectives.
