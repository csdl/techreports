...
representing within-classes variance in order to achieve a good accuracy. 
...
In this work, we address these limitations by proposing an alternative 
to 1NN algorithm that provides a superior interpretability, learns efficiently from a
small training set, and has a low classification computational complexity.
...
The first is the time series shapelets technique,
...
The second relevant technique is the 1NN classifier built upon the 
...
and is obtained by extraction, transformation with Symbolic Aggregate approXimation 
(SAX, \cite{sax}), and counting of occurrence frequencies of short overlapping 
...
It was shown by the authors, that BOP has several advantages: it has a linear
...
training time series into the vector space and computes \textit{tf$\ast$idf}
ranking.
...
patterns with a single bag and \textit{tf$\ast$idf} subsequence ranking, 
allow SAX-VSM to achieve a high classification accuracy and provide 
...
SAX algorithm, however, requires two parameters to be provided as an input and there is 
no efficient solution for parameters selection known to the best of our knowledge. 
...
Next, we employ the \textit{tf$\ast$idf} weighting scheme for each element of this 
matrix in order to transform a frequency value into the weight coefficient. 
...
The algorithm begins by transforms all labeled time series into symbolic representation. 
For this, it converts time series into SAX representation configured by four parameters: 
the sliding window length (\textit{W}), the number of PAA frames per window (\textit{P}), 
...
In our evaluation, we followed a train/test split of the data exactly as provided by UCR or 
other sources. Train data were used in cross-validation experiments for optimization of SAX
parameters and numerosity reduction strategy using DIRECT. 
Once selected, optimal parameters was used to assess SAX-VSM classification accuracy 
which is reported in the last column of the Table \ref{perf_table}.
...
%SAX-VSM\\\midrule
\hline
Data set & Nb. of classes & 1NN-Euclidean & 1NN-DTW & Fast Shapelets &  Bag Of \mbox{Patterns}
& SAX-VSM\\
\hline
%\midrule
...
experimentation.
We used CBF \cite{cbf} in order to investigate and compare the performance of SAX-VSM and 1NN
...
achieving 61\% of classification accuracy. However, as we pointed out above, DTW provided a
