...
Thus, it is often used as an embedded procedure within a number of other techniques, notably 
shapelets \cite{fast-shapelets}, which utilize the low-bounding property of SAX approximation for 
performance improvement.

Algorithm consists of following steps: at the first step, the time series (or a sub-series) are
z-normalized (normalization by energy) \cite{goldin_kanellakis}. 
Next, the dimensionality of the time series is reduced by PAA, 
Piecewise Aggregate Approximation \cite{paa}. 
In PAA, times series is divided into \textit{n} equally sized segments, 
and the the points within each segment are approximated and replaced by their mean values. 
Finally, in the third step of SAX algorithm, the PAA representation of the time series is converted 
into the string by mapping PAA coefficients into $\alpha$ equiprobable regions dividing 
distribution space. The construction of these alphabet lookup tables is based on the assumption 
that normalized time series tend to have Gaussian distribution \cite{larsen_marx}.  

Note, that in further SAX development \cite{hot_sax}, for its application to problem of discords
and motifs discovery, streaming data analyses and clustering \cite{streaming_sax}, authors 
proposed a sampling strategy and a distance function specifically designed in order to avoid 
trivial and degenerative solutions.  In our work we experimented with this combination and 
found it useful in the search of optimal parameters. For some datasets, it significantly reduced
...
for disambiguating class entities \cite{salton}. Similarly to IR, we define and 
use \textit{bag of words}, \textit{corpus}, and \textit{sparse matrix} in our workflow. 

Terms \textit{bag of words} and \textit{document} are used as an abbreviation of 
unordered collection of SAX words. While in IR these are usually different, and a
\textit{document} usually presumes certain words ordering - we use both terms 
interchangeably. 
...
workflow precisely. Exactly as in IR, we use the term \textit{corpus} for a 
structured collection of bags of words. 
...
classes, its rows correspond to \textit{all seen} SAX words, thus, many elements are zeros - 
...
denominator $df_{t}$ is a number of documents where the term $t$ appears.
...
\section{Description of the SAX-VSM classification algorithm }
...
further improve accuracy for a number of datasets. At this point we unable to comment
on this fact and leave this direction for future work.
...
First of all, by combining all SAX words observed in all time series which belong to one class into a single bag, SAX-VSM manages to exhaustively capture an intraclass variability. Intuitively, this improves the algorithm's classification quality for those time series which possess low similarity to labeled series at the full length, but possess some unique patterns characteristic for the class. 
...
CBF data: SAX-VSM performs much better with limited amount of training samples (LEFT). 
SAX-VSM is much faster in classification phase and comparable with 1NN classifier when training time
is accounted (RIGHT).
------ GREEN legend:  SAX-VSM with TRAIN}
...
to determine whether or not she was holding a gun or imitating the action.

Similarly to reported results, SAX-VSM was able to capture both features which are distinguishing
GUN/POINT classes as shown at the Figure \ref{fig:shapelet-like-patterns}. The most weighted SAX
