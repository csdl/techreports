...
%\cite{logical} 
classifier and the shapelet transform classifier \cite{bagnal}. We selected these particular 
...
promising results. 
In one series of experiments, we introduced a Gaussian noise into unlabeled series,
varying its standard deviation up to the maximal data amplitude. SAX-VSM performed significantly 
better than 1NN Euclidean classifier (Fig.\ref{fig:corrupted}) in this experiment 
with medium to high noise presence. 
In another series of experiments, we randomly replaced up to fifty percent of an unlabeled
time series span with a random noise. Again, SAX-VSM performed consistently better than 
1NN Euclidean classifier regardless of a training set size which we varied from five to
one thousand time series of each class (Fig.\ref{fig:corrupted})
(we used 10K time series of each class for testing in both experiments). 
