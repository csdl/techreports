...
in greater detail, while here we present its overview and outline its specificity and main 
differences from other techniques.

The training of the SAX-VSM classifier is relatively computationally expensive 
...
Only the set of $N$ real-valued vectors is retained after the training for classification. 
...
time series is assigned to a class whose weights vector yields a maximal cosine 
similarity value. 
...
discovery of multiple shapelets provides better resolution and intuition into interpretability of
classification. However, as the authors noted, a time cost of multiple shapelets discovery
...
metrics - we relied on \textit{tf$\ast$idf} weights vectors and the cosine similarity metrics. 
