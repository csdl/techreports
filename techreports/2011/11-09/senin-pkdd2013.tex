...
phase and the classification. In the following sections we shall explain the algorithm 
in greater detail, while here, we present an overview of our method and outline 
its specificities and differences from other techniques.
...
processed with \textit{tf$\ast$idf} and can be also discarded - only a set of $N$ 
resulting real-valued weight vectors is retained after the training phase for classification. 
Note, that the construction of a single bag of words for each of the training classes 
is one of the main differences of SAX-VSM from other previously published 
techniques \cite{bag_patterns}.

The use of cosine similarity for classification is another main difference of SAX-VSM 
from competing techniques. SAX-VSM facilitates a fast classification, which consists 
of the conversion of an unlabeled time series into a terms frequency vector, and its 
successive classification by the computation of $N$ cosine values. The unlabeled 
time series is assigned to a class whose weight vector yields a maximal cosine 
similarity value. 
...
into SAX representation configured by four parameters: the sliding window
...
As we mentioned, each of the extracted with sliding window sub-series is normalized
before being processed with PAA.
%\cite{goldin_kanellakis}. 
If, however, the standard deviation value falls below a fixed epsilon threshold, we do not 
apply this normalization.

By applying this procedure to all $N$ training classes, we build a corpus of $N$ bags, 
which, in turn, we process with \textit{tf$\ast$idf}. This procedure results in $N$ 
real-valued vectors of equal length representing training classes. 
...
The unlabeled time series is assigned to the class whose vector yields the maximal cosine 
similarity value.
...
In order to overcome this limitation, we employ the rounding of reported solution values 
