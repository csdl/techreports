...
machine learning, statistics, signal processing, environmental sciences, and 
chemometrics.
...
classification performance of these techniques:
while shape-based similarity methods virtually 
...
Additionally, in the context of our work, it is worth to note, that 
classification techniques can be further divided into exact or approximate.
Former are based on an exact metrics, and typically are brute force algorithms 
which guarantee results through an exhaustive search, 
while the latter are usually built upon an approximate representation of 
time series, or upon an approximate similarity metrics, and do not guarantee 
the global correctness. Nevertheless, while allowing a higher error rate, approximate 
techniques effectively reduce the search space and enable indexing,
thus dealing with large data sets much more efficiently.
...
Recently, as a possible alternative to this traditional categorization, two interesting 
techniques, relevant to our work, were proposed. The first technique is 
the time series shapelets algorithm which was introduced in \cite{shapelet} and gained 
popularity. A shapelet is a short time series ``snippet'', that is a representative of class
membership and is used for a compact decision tree construction, facilitating class 
...
For classification, it relies on the similarity measure between a shapelet and a full time
series defined as a distance between a short shapelet and a closest subsequence in 
the series when measured full by the normalised Euclidean distance. 
It exhaustively searches for a best discriminatory shapelet on data split via 
an information gain measure, 
This technique, , and, potentially, combines the superior precision
of 
shape-based exact similarity methods , and the high-throughput classification capacity 
and efficiency of feature-based approximate techniques. 
...
This was later extended into a computationally less expensive ($O(nm^{2})$) 
shapelets-based technique which finds and approximate solution and based on SAX
\cite{fast-shapelets}.

The second relevant to our work approach is based on the structural similarity 
time series and called Bag-of-Patterns (BoP) \cite{bag_patterns}. 
In BoP, the time series is converted based on SAX representation the small overlapping subseries
which 
are equated to the words in a document, and counts the occurrence
of these words to create a vector of word frequencies. By doing
so, it paves the way for the comparison of two time series in
the same way that you would compare two documents in a
bag-of-words style, by examining the histograms of their word
frequencies.
In BoP, each time series is converted into a frequency
vector of length as . The vectors are calculated by having a
window of length w slide over the entire time series and
converting the subsequences into a SAX representation, a
word. The frequency of the “SAX words” is stored in the
vector. Identical consecutive words are only counted once
when numerosity reduction is set, to avoid matching based
on recurring patterns (like stop words in a document). Figure
2 displays the frequency vectors for a collection of m time
series. Distance between the vectors can be calculated using
any conventional distance metric.
BoP outperforms existing techniques for classification,
clustering and anomaly detection of univariate time series
...
However, to date, despite recent progress in classification approaches, the best algorithm 
in overall performance is the simple nearest neighbor classifier - that is accurate,
robust, and depends on a very few parameters \cite{review, benchmark, comparison, classifiers}.
But while possessing all these qualities, 1NN technique has a number of disadvantages: 
it does not offer any insight into the classification results and needs a significantly large 
training set in order to achieve a good accuracy.
