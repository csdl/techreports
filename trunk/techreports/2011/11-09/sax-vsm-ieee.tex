...
In the context of our work, it is worth to note, that classification techniques can be 
further divided into exact or approximate.
While exact techniques are based on an exact metrics, and typically are brute 
force algorithms which guarantee results through an exhaustive search, 
approximate techniques are usually built upon an approximate representation of 
time series, or upon an approximate similarity metrics, and do not guarantee 
the global correctness \cite{logical}. Nevertheless, while allowing a higher error 
rate, approximate techniques effectively reduce the search space and enable indexing,
thus dealing much more efficiently with large data sets.
...
Recently, as a possible alternative to this traditional categorization, 
``time series shapelets''  were introduced \cite{shapelet} and gained popularity. 
A shapelet is a short time series ``snippet'', that is a representative of class membership, 
which can be used for a decision tree construction, facilitating class disambiguation and 
interpretability \cite{logical}.
This approach is exact only for a length of a shapelet, and, potentially, combines the superior 
precision of shape-based exact similarity methods, and the high-throughput classification 
capacity and efficiency of feature-based, approximate techniques. 
...
While a less computationally expensive ($O(nm^{2})$) shapelets-based technique was 
proposed, it is an approximate solution \cite{fast-shapelets}.
