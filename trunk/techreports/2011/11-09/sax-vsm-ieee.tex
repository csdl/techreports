...
Former are based on exact metrics, and typically are brute force algorithms 
...
the global correctness. Nevertheless, while allowing a higher error rate,
...
Recently, as a possible alternative to this traditional categorization, two relevant 
to our work techniques, were proposed. The first technique, featuring superior 
interpretability, is the time series shapelets algorithm which was introduced in 
\cite{shapelet} and gained popularity. A shapelet is a short time series ``snippet'', 
that is a representative of class
...
Classification is built upon the similarity measure between a branching shapelet 
...
This technique is exact to the shapelet size, and, potentially, combines the superior 
precision of shape-based exact similarity methods, and the high-throughput 
...
approximate shapelets-based algorithm which is based on SAX 
approximation and indexing \cite{fast-shapelets}.
...
based on the Euclidean distance, DTW, or tf*idf. It was shown that BOP
has a several advantages: it is rotation-invariant, it considers local and global 
structures simultaneously, and it provides an insight into pattern distribution 
