...
easily tuned for a particular problem by the choice of distance measure, 
approximation technique, or smoothing \cite{comparison}. 
...
provide any insight into the classification results.
...
Section \ref{prior} provides background into the existing algorithms categorization and 
discusses relevant work. 
...
ECG200          &2   & 0.120   & 0.230           & 0.227     & 0.140   & \textbf{0.09} \\
...
For small training data sets, SAX-VSM was found significantly more accurate than 1NN Euclidean
classifier. However, by the time we had more than 500 time series in our training set, there was no
statistically significant difference in accuracy (Fig. \ref{fig:precision-runtime}). 
As per the running time cost, due to the comprehensive training, SAX-VSM was found to be more
expensive than 1NN Euclidean classifier on small training sets while dealing more efficiently with
large sets.
...
faster than 1NN Euclidean classifier (Fig. \ref{fig:precision-runtime}).
...
words and curves for dictionary sizes of each of CBF classes reflected significant saturation 
...
accounted 23'569 value, Fig. \ref{fig:venn}. In our opinion, this result reflects two
specificities: the first is that the diversity of words which are possible to extract from CBF
dataset is quite limited by its classes configuration and by our choice of SAX parameters. The
second specificity is that idf (Inverse document Frequency, \ref{formula:idf}) limits the growth 
...
\begin{figure}[t]
 ...
 (left panel; the noise level varies up to 100\% of signal value (6.0),
