...
discovery based on three existing techniques - 
Symbolic Aggregate Approximation (SAX), TFIDF weighting, and Vector space model. 
...
which is simple, and depends on a very few parameters. 
However, while being virtually unbeatable in precision, kNN algorithm 
...
the particular time-series was assigned to a particular class except their
mutual distance metrics.

As an alternative to kNN algorithm, recently, time series shapelets were
introduced [cite]. Instead of iteratively looking for a closes neighbor among all 
known instances, shapelets-based approaches classify unknown sequences based 
on their inclusion or exclusion [cite logical shapelets] of known shapelets. 
While demonstrating similar or superior to kNN algorithms performance,
...
which is capable of building a well-performing classifier by automatically
recognizing and ranking of characteristic to classes features.
...
\section{Yep. That old template stuff is still there}
