...
optimization with DIRECT algorithm for SyntheticControl dataset. Left panel shows all points
sampled by DIRECT in the space $PAA*Window*Alphabet$ - red points correspond to high error 
values in cross-validation experiments, while green points correspond to low error values; note
the green points concentration at $W$=42.\\ 
...
\begin{figure}[tbp]
   \centering
   \includegraphics[width=115mm]{figures/shapelet-patterns.ps}
...
OSULeaf dataset. These patterns align with well known in botany discrimination techniques by lobe
