...
%
\titlerunning{Interpretable Time Series Classification Using SAX and Vector Space Model}  
...
approach combines best of two categories - the superior precision of shape-based 
...
class. 
However, instead of iterative search for these (as in shapelets case), our algorithm finds, 
and weights by ``importance'' all potential candidate subsequences at once. 
In addition, being built upon SAX approximation that facilitates smoothing and elasticity, 
...
By utilizing a sliding window subsequence extraction and SAX, our algorithm transforms 
...
that converges relatively quickly and yields a deterministic, optimized solution. Moreover, it
...
At the first step of the algorithm, $T$ is z-normalized (to unit of standard deviation) \cite{goldin_kanellakis}. 
At the second step, a dimensionality of the normalized time series is reduced from $n$ to $w$ by
obtaining its Piecewise Aggregate Approximation (PAA) \cite{paa}; for this, the normalized time series is
...
This table is pre-built by defining a set of breakpoints that divide the normalized time series distribution 
...
strategy and a distance function and found that for most of the data sets, inclusion 
...
out of thirty one data sets that we have used for the performance evaluation (see
\cite{jmotif}).
%
...
parameters that were used in the training phase. 
...
\begin{figure}[t]
   ...
   red points corresponds to high error values in cross-validation experiments, while green points
...
In our experimentation with many data sets, we observed, that the dimensionality of \textit{tf$\ast$idf} 
...
\begin{figure}[t]
   ...
   \caption{Examples of best-weighted patterns from each class of \textit{GunPoint} data set. 
...
\textit{OSULeaf data set}. These patterns align with well known in botany discrimination techniques
...
\begin{figure}[t]
   ...
   \caption{A comparison of hierarchical clustering application to a subset of three
...
First, while Vector space model naturally supports processing of bags of words composed 
...
We are experimenting with two candidate implementations: the first is based on a
...
dimensions. The preliminary results on synthetic data sets look promising and we are
