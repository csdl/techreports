...
%The BOP technique includes extraction of short overlapping subsequences (patterns) along the time series, 
%transformation them with Symbolic Aggregate approXimation (SAX) [12], and counting  frequencies of SAX  words.
...
\end{equation} 
where $t$ is the term, $d$ is a bag of words (a document in IR terms), 
...
and their error function value. This procedure continues interactively until the error function
converges.
...
in this case, algorithm converged after sampling just 130 out of 13'860 possible parameters 
combinations (\textgreater100x speedup).
...
unrealistic training set sizes - up to $\normalsize{10^{9}}$ of instances for each of CBF classes.
...
\begin{figure}[t]
   ...
   (the random noise level grows up to 100\% of CBF signal value)
   }
