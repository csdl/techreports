...
Data from system logs indicates that Telemetry and Software ICU were frequently used during the evaluation period. Telemetry is invoked much more frequently than SICU (see \autoref{fig:usage-overtime} and \autoref{fig:per-student-all}). The Telemetry invoke stream and SICU invoke telemetry stream are quite similar in ``shape''(see \autoref{fig:usage-overtime}) that we can confirm drill-down feature of the Software ICU is useful and contributes to the high use frequency of Telemetry analyses. Compared the log data to the questionnaire response in \nameref{Q11}, most students' reported use frequencies match their actual usage, expect the three lowest students in \autoref{fig:per-student-all}, who said they use Software ICU 2 to 3 times a week or more, but actually only half as much as they claimed. The reported frequency of SICU and Telemetry are similar while telemetry's analysis count is much higher than SICU's. But this match the native of these two analyses: SICU shows the overall summary of the project's health and no need to run more than once a day, while Telemetry shows detail of a vital sign and would often be run multiple times in every use.

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=\textwidth]{per-student-all} 
   \caption{Analysis count on a per-student basis during the evaluation period.}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering     
   \subfigure[Usage of each Telemetry analysis]{
