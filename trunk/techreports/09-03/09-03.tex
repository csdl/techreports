...
First, for better present Coupling data, it should provide a coloring method that classify values between two thresholds to be good, the values exceed thresholds within certain amount, either by smaller or bigger, to be average, and other values to be bad. The trend for this will prefer stable only. 

Second, SICU can provide an overall health rating of each project, based on their metrics. It will be a fussy rating like five stars, and user will be able to choose correspondent metrics and their weights. It will be more useful when the number of projects become bigger.
