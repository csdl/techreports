...
\item The both the installation and sending sensor data was easy.  However, tracking down whenever there is a problem with the sensor is not so easy.  A troubleshooting page in the near future?
...
\item Some the installation instructions could benefit from "write once, use many times" as they're repeatitive, which causes some people to start glossing over the instructions and then there's a couple that are slightly different and people (like me) won't notice the difference.
...
The data indicates that eclipse sensor was easy to install. Ant sensors were little more difficult to install. The most difficult one is the SVN sensor, which require configuring the Hudson, a continuous integration engine. 

The major problem cause installation difficulties is the documentation. Though we privoded guides for every components, some students point out that the documentations are too distributed and too long. Some guides are too similar because of copy and paste that slightly but important difference will be easier ignored. A more straightforward and brief walkthrough or even an auto installation package are most expected.

Regarding the collection of data, no big problem is reported. 

The server side installation was not evaluated because the students are not required to configure the server. They were all using the well-configured public server.

\subsection {Conclusions regarding overhead of use}
The overhead of collecting data and running Hackystat analyses were in a middle level. The comments indicate that the most concerned problem was the speed, both of collecting data and of running portfolio analysis.
...
The data indicates that both telemetry and Software ICU were frequently used during the evaluation period. The top three useful vital signs the students considered are DevTime, Coverage and Commit. 

When using telemetry, Member Devtime, Member Commit and Member Build are the major analyses that students paid attention on. More than half of the students said they were trying to find out the member devtime of themselves and/or their groupmates. This may partially because the major component they were using was Software ICU and the member level analyses are only found in Telemetry. However, from the vital sign popularity, we believe this is another evidence that the students paid more attention on DevTime and Commit statistics than others.

When using Software ICU, 10 out of 18 responses said the Software ICU was accurately reflecting the health of their project via colors. 4 responses disagree that was accurate, mainly arguing that high and/or increasing Churn and Coupling are not necessarily bad. Most students thought Hackystat did help them improve their performance either in programming or teamwork or both.

We can conclude that, though there are still some deficiencies about coloring methods and potential bias of the data cause by the native or the way we collect it, Hackystat does achieve its goal to provide users useful tools to help them understand and improve their programming procedure.

\subsection {Feasibility in a professional software development context}
The data indicates that most students think it is at least somewhat feasible to use Hackystat as a professional developer. Most comments indicate that Hackystat would be helpful in professional settings. There are some comments concerned about the potential bias of analysis data of Hackystat, which may not be complete to determine a project's health state.
...
First, in 2007, the Hackystat system underwent a complete rewritten for a new version: Hackystat v8. Hackystat v8 now is organized as a collection of loosely coupled software services that communicate using REST architectural principles. The new Hackystat v8 in 2008 was much more powerful in functionality than any previous version of Hackystat. Thanks to the new web-based architecture, it was also much easier to add new functionality, as well as new user interface, by adding new services, contributing to higher configurability in functionality. Therefore, we expect that the utility of the system should be higher than the old versions.
...
The next section presents a comparison of the data from 2003, 2006 and 2008. All data has been convert to percentage in order to ignore the difference of the number of participants. At this time, no statistical tests for significant difference have been performed.  Thus, any differences claimed between the data sets based upon the ``shape'' of the histograms are tentative and await statistical confirmation. 
...
The empirical data indicates that the installation difficulties of both Eclipse sensor and Ant sensors increased in 2008, especially the Ant sensors. Compared to Hackystat in 2006, the major shortage in installation is the absence of the installer. The distributed documentations instead of a single user manual is another contributor to the difficulty increase. Also the setup of SVN sensor and daily build task that collects productive data involves the configuration of Hudson and thus further increase the installation/configuration difficulty.
...
After the overhead of running Hackystat analyses increased slightly in 2006, it further increased in 2008. But there are no feedback about what factor lead to this high overhead. One of the major factors might be the processing time of SICU analysis, which usually took several minutes to finish. 
