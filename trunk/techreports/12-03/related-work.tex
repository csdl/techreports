...
This chapter examines related research in this area. Section 2.1 investigates the recent development of ``gamification''. Section 2.2 looks at the applications of ``serious game''. Finally, section 2.3 examines the game related metrics and analytics of game systems.
...
\subsubsection{Microsoft RibbonHero - Making You Better Your Job} 
...
\subsubsection{RecycleBank - Making the World Sustainable}
...
\subsubsection{Why Games and Now}
...
The following sections examine a few popular games and genres to understand why game has such power in our society.

\subsubsection{Ancient Games}
...
\subsubsection{Angry Birds: the Additive Casual Game}
In today's tech world, no gaming platform is completed without the new star game Angry Birds. This simple game has been downloaded over 300 million times. It has been played roughly 200 million minutes a day across the world, that is 1.2 billion hours a year. According to Nieman Journalism Lab, all person-hours spent creating and updating the entire wikipedia total about 100 million hours \cite {niemanlab2010}. That is half day of the Angry Birds play time. 

Why is this seemly simple game so massively additive? Charles Mauro discussed the cognitive ways of Angry Birds in Human factor engineering for the sake of answering the more ``important" real world question, ``why users don't find their company's software or product engaging?". According to him, the game has a simple engaging, easy to learn interaction model and incremental complexity with anticipated rewards \cite {Mauro2011}.

\subsubsection{FarmVille: Social Games}
With the motto  ``Connecting the world through games", Zynga quickly becomes one of the top game companies, catching up to the more traditional establishments such as EA and Activision Blizzard. Helped by the social network platform Facebook, its FarmVille and CityVille are among the most popular games in the world. FarmVille has 71 million active players. Although it is free to play, Zynga is estimated to generate \$50 million in revenue from the more engaged players who buy virtual goods in game. Phil Michaelson pointed out 8 tactics that FarmVille uses to design for engagement \cite{Michaelson2010} :
...
\subsubsection{World of Warcraft : Alone together in MMORPG}
...
\subsubsection{A Game Layer On Top Of The World}
Seth Priebatsch, a young CEO at startup SCVNGR, gave a great TED talk titled ``The game layer on top of the world'' \cite {Priebatsch2010ted}. His main message is: ``Last decade was the decade of social. This next decade is the decade of games''. ``social layer's purpose is to connect; a game layer is to influence''.

How to build a game layer? He claimed that seven game mechanics ``can get anyone to do anything'', and listed four of them in his talk: (1) Appointment dynamic. (2) Influence and status. (3) Progression dynamic. (4) Communal discovery.

\subsubsection{Game Based Marketing}
...
Csikszentmihalyi describes that there are seven core components of flow, as  summarized in Table 2.1. These components can be broken into two categories: conditions and characteristics. Conditions must be achieved before flow can be reached. Characteristics occur while a person is in flow, even though they may be unaware of it.
...
Bartle's player type model has been the basic for understanding the player motivation. Amy Jo Kim applied the model in her gamification approach by overlaying social actions from the game on top of the player types \cite {Kim2010}, as shown in Figure 2.8.

\begin{figure}[htbp]
	\centering
		\subfigure[Bartle's Player Types]{\label{fig:player-types}\includegraphics[height=2.0in]{bartle-player-types.jpg}}
		\subfigure[Kim's Social Actions for Players]{\label{fig:social-action}\includegraphics[height=2.0in]{kim-player-types.jpg}}
		\caption{Player Types}
		\label{fig:play-types}
\end{figure}

\subsubsection{Fogg Behavior Model in Persuasive Design}
...
After his inspiring talk in DICE 2010, Jesse Schell and Bryan Reynolds (Zynga chief designer) discussed about ``Gamification vs. Gameplay". They are arguing in a very basic level of the definition of gamification. Brian considered ``Gamification is where you use game elements to try to get people to do stuff they don't want to do", while Schell responded that ``It's a problem solving situation that you enter into because you want to". Reynolds argued ``Everyone who has tried to use game mechanics to improve their marketing has only managed the most basic concepts''. Schell responded that this was the developers', not the concept’s fault \cite {scimeca2011}.
...
\subsubsection{Gamification is Bull*it}
Georgia Institute of Technology professor and game designer Ian Bogost called gamification efforts ``exploitation-ware" that is being ``invented by consultants'' and claimed that gamification is bullshit \cite {bogost}. Gamification, he argued, ``gets games wrong, mistaking incidental properties like points and levels for primary features like interactions with behavioral complexity". In the GDC 2011 gamification debate, he contrasted that ``To take something like games, which are complicated, and substitute it out for points and badges is a very efficient way to get a hot culture commodity into your product". 

\subsubsection{Poinstification}
...
\subsection{Gamification Design: HOW}

\subsubsection{Gamification 1.0}
...
\begin{figure}[htbp]
	\centering
		\subfigure[Satisfies Human Needs (Bunchball)]{\label{fig:human-needs}\includegraphics[width=2.5in]{human-needs.png}}
		\subfigure[Basic Mechanics]{\label{fig:basic-elements}\includegraphics[width=2.5in]{basic-element.jpg}}
		\caption{Gamification 1.0}
		\label{fig:gamification-1-0}
\end{figure}


\subsubsection{SCVNGR Game Mechanics Playdeck}
Seth Priebatsch stated that you can get anyone to do anything with seven game dynamics \cite {Priebatsch2010ted}. Beyond the seven, they actually have 47, illustrated in a play deck \cite{Biggs2010}. It works as a set of flash cards. SCVNGR employees are instructed to memorize these game dynamics cards and find places where they exist in their applications. Most of the mechanics in the deck are listed in the Appendix of this review.
...
\subsubsection{Four Keys to Fun}
...
\subsection{Gamification Service and Platform}

\subsubsection{Commercial products and services}
...
\subsubsection{Mozilla - Open Badges Infrastructure}
...
\subsubsection{Open Source Gamification Platform}
...
Another great example is the "Piano Staircase" created by Volkswagen Sweden and ad agency DDB, installed in a metro station in Stockholm \cite {funtheory2009}. The design is to make the staircase next to the escalator look and respond like a piano keyboard, so that every step on the stair will generate different piano sounds every time a commuter walked on it. Observation indicates that 66 percent more people chose the staircase over the escalator, a good example of a "Fun Theory" design for persuading and encouraging energy-efficient behavior.
 ...
 \begin{figure}[htbp]
	\centering
		...
		\subfigure[Piano Stair or Escalator]{\label{fig:ixd-pianostair}\includegraphics[scale=0.35]{ixd-pianostair.png}}
...
using game metrics for analysis of player's experience in a quantitative approach \cite {ducheneaut2006alone}. They reported the relationship of playing time and leveling in the MMORGs, as shown in the following figure:
