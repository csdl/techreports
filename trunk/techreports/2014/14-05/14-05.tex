...
In this work we focus on the discovery of structural anomalies and address the above limitations by proposing a technique capable of efficiently detecting anomalies of variable lengths. The proposed algorithm relies on the grammar induction procedure which, once applied to a string obtained by symbolic time series discretization with SAX \cite{sax}, builds a hierarchical structure of context-free grammar rules, each of which maps to variable-length subsequences of the input time series. By analysis of the grammar's hierarchical structure, our algorithm efficiently identifies symbols that are rarely used in grammar rules and whose corresponding subsequences we relate to potential anomalies. %subsequences
%\fxnote[inline]{(Arnold) we can define a top-k ranking criterion as follows: for anomalies of length greater or equal to L, rank these anomalies based on their rule density in ascending order, and output the top K elements of the ranked set. The L parameter provides additional flexibility by allowing anomalies to be contextualized to the application's their unique requirements. For example, if we have an application where it is costly to obtain data samples and each sample has little noise/uncertainty, then setting l=1 may make sense. However, for fast streaming and noisy data, setting a large l would make more sense.} 
...
%\fxnote[inline]{(Arnold) The definition above needs to be more specific and may be unclear to readers unfamiliar with HOTSAX, perhaps we can borrow additional text from the HOTSAX paper to make it clearer.}\fxnote[inline]{(Pavel) we need to formally define some sort of alternative to discord to use it throughout the paper. Discord definition is useful for discussing of previous work though.}\fxnote[inline]{(Arnold) Yes, an alternative definition is needed. Jessica made an excellent suggestion on that in one of her prior email. We can further specify the "general anomaly based on grammar coverage" to one that has low probability of occurrence with respect to its order in the time series i.e., context. More about this in my comment below..}
...
%-- \textbf{we consider not real-valued subsequences but the substrings of string that represents the discretized time series.} 

%\fxnote{(Jessica) I'd remove this bolded sentence to differentiate ourselves with other SAX-based anomaly detection techniques. For example, VizTree also finds anomalies based solely on the frequency of SAX strings. Another side of the argument is that, looking at SAX strings alone does not guarantee exact answers, hence the different heuristics proposed for distance computations, early abandonment, etc in the literature. In that regard, I don't think we gain advantage by emphasizing this. What we want to emphasize is the sentence below, that we don't just look at single SAX strings, but a series of them with some hierarchical patterns discovered by grammar.}  

%\fxnote[inline]{(Arnold) The statement above on defining anomalies as the subsequence with the "smallest coverage by the grammar rules" needs to be further qualified. To me, what we're really trying to say is the following: subsequence M is an anomaly if an only if the conditional probability of M given its prior temporal neighbor observations/data is the minimum out of all other subsequences. In other words, an anomaly is a subsequence that has the lowest probability of occurrence with respect to its order in the time series. How then is this related to discords? Since M has the lowest conditional probability, this should mean that M prefixed with it's prior temporal neighbors (not only M) should be a discord. We need to think about this some more since this relationship will dictate the metric by which we quantify the effectiveness of SAX Sequitur.}
...
%\fxnote[margin]{(Susan) What are augmented tries? Is it a specific method or just different uses of Haar wavelets? (Pavel) I believe that it is a highly efficient (fast to query) data structure.}
...
%In previous work, we introduced a linear-time, variable-length motif discovery algorithm based on grammar induction \cite{grammarviz}. Our current work is motivated by the fact that the anomaly detection problem can be viewed as inverse to motif discovery. Since motifs are frequently occurring patterns whereas anomalies are rare patterns, we propose a similar, grammar-based technique that differs in the pattern discrimination part. As we shall show, the proposed solution is capable of addressing the aforementioned limitation of identifying variable-length discords in time series without any prior knowledge. It also does not need additional parameters such as the anomaly shape or a support threshold. While it still requires SAX discretization parameters to be specified, from our experience, it successfully tolerates non-optimal discretization settings in a wide range as they are effectively mitigated by the grammar induction procedure. In addition, we present \textit{an approximate discord discovery algorithm that does not need any distance computation}. \fxnote[inline]{not sure how to make these clear...} 
%\fxnote[inline]{(Crystal) Short, summary description of exact method was not mentioned. Also, maybe some more description or experimental proof can be provided to show how SAX parameters are more robust. There are a lot of parameters and thresholds that still need to be set for the SAX/Sequitur method, and a more convincing explanation/justification would be beneficial.} \fxnote[inline]{(Pavel) cut it short}
...
%Dimensionality reduction is embedded into the SAX discretization process as $w$ is almost always much smaller than the original sequence length $m$. While this is a desirable feature, in some cases we also need the capability of capturing local patterns, which may be lost if the global SAX compression ratio ($m/w$) is too high. 
...
%\fxnote[inline]{(Arnold) The SAX compression ratio: should it be ($m/w$) instead? right it should be that.}

%\begin{figure}[t]
%   \vspace{-0.5cm}
%   \hspace{-0.8cm}
%   \centering
%   \includegraphics[width=80mm]{Figure2_SAX.eps}
%   \caption{
%   Example of SAX for a time series. 
%   The time series above is transformed to the string $cbccbaab$, 
%   and the dimensionality is reduced from 128 to 8.}
%   \label{fig:sax}
%   \vspace{-0.5cm}
%\end{figure} 
...
%For subsequence discretization, we need to determine which subsequences to use and in which order. It is generally a good idea to consider overlapping subsequences as we have shown in \cite{lin_motifs}, since non-overlapping subsequences will likely produce coarse discretization or, worse, cause interesting patterns to span multiple segments, resulting in a significant loss of information. 

%\fxnote{(Susan) Does the alphabet size have to be the same for each segment or can you have more letters for segments with greater variability? (Jessica) Currently we use the same parameter for all segments, but adaptive alphabet is an interesting idea (though I think it's been done in iSAX for indexing).}
...
%\fxnote{(Jessica) Added a couple sentences on digram. (Pavel) Thank you, it is clear.}
...
In this grammar, $R1$ concurrently covers intervals of different lengths: $S1_{[1:3]}$ of length 3, and $S1_{[6:9]}$ of length 4 respectively. The potential anomalous substring ``$abb_{4}$ $acd_{5}$'' has length of 2. Recall each SAX string corresponds to a time series subsequence, so these intervals correspond to subsequences of variable lengths. %\fxnote{(Jessica) Added the sentence "Recall..." for clarification.}
...
%\fxnote[inline]{(Pavel) re-phrased. (Jessica) This sentence may raise some questions so maybe we should be more explicit. It might be more intuitive to reverse this: rather than saying that a time series point corresponds to a SAX string, say that a SAX string corresponds to a point (or more precisely, a subsequence that starts at this time point)?}
...
%\fxnote{(Pavel) We can add about the thresholds allowing the anomaly extension right here.}
...
%\fxnote[inline]{(Jessica) Pavel, would you please add more results for the approximate algorithm?(Arnold) Since a major contribution of this work is efficiency, it would be very helpful to elaborate on the derivation of the linear time and space costs.(Pavel) I've changed the text.}
...
% By using SAXSequitur the user is able to visually and interactively examine Sequitur rules. Selection of a rule in \textit{Sequitur rules} table highlights all its occurrences on the time series and displays all the subsequences corresponding to the rule in a \textit{Rule Subsequences} window. Both chart panels: \textit{Data display} and \textit{Rule Subsequences} are capable of interactive zooming either with the mouse or through use of a pop-up menu. By selecting the \textit{Rule density} view, the user is able to visually examine Sequitur rule density which highlights all potential discord locations as shown at Figure \ref{fig:densityECG}.
...
%\fxnote{(Jessica) We should emphasize this, maybe mention this earlier in the paragraph. Also, does the first algorithm find all anomalies as well?; (Arnold) Pavel, to answer your question on the experiment setup, we should first think about the relationship between discords and coverage-based anomalies.}
...
%\fxnote{(Jessica) Pavel: Please add to each experiment the SAX parameters used to ensure reproducibility (I've seen many reviewers complain about irreproducibility of experiments, e.g. lack of details, parameters, etc, and that alone could be a reason for rejection. Also please check if the parameter ranges I mentioned above are correct.}
...
%\fxnote[margin]{(Susan) Superior performance to what?.}
