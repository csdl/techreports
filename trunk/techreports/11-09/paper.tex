...
Following previous research I partitioned the change trails by the time of the day and by 
time periods of a week, two weeks and a month. 

\subsection{Token-based distance metrics}
In my experiments I used two similarity metrics specifically designed for multisets comparison. These are token-based distance metrics:
The Jaccard similarity between two sets $S$ and $T$ which is simply 
\begin{equation}
J_{\delta}(S,T) = \frac{|S\cup T| - |S\cap T|}{|S\cup T|}
\end{equation} 
and the $TFIDF$ or $cosine$ $similarity$ which can 
be defined as 
\begin{equation}
...
\end{equation} 
is a cosine normalization of $tf\ast idf$ (product of token frequency and inverse document frequency):
\begin{equation}
...
By using $IDF$ we put more weight on words which occur less frequently among others in order to better separate 
%% See \href{http://en.wikipedia.org/wiki/TFIDF}

\section{Results}
\subsection{Kernel-OMAP contributors partitioning}
For the 
