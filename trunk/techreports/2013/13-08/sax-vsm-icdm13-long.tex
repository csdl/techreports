...
Another limitation is its need for a significantly large training set, that represents a 
...
Thus, the demand for a simple, efficient, and interpretable classification technique 
...
small training set, and has a low computational complexity in classification.

The paper is structured as follows. 
Section \ref{prior} provides background into the existing algorithms and discusses relevant work. 
...
(patterns) along the time series.
...
pattern (a SAX word) occurrence frequency vector. 
These vectors are classified with 1NN classifier built upon Euclidean distance, 
or Cosine similarity on raw frequencies or with \textit{tf$\ast$idf} ranking. 
...
IEEE World Congress on Computational Intelligence website \cite{ford}, the 
...
The performance of SAX-VSM for the rest of the data sets can be found 
at the project homepage (\url{https://code.google.com/p/jmotif/}).
