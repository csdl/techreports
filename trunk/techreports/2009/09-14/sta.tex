...
Discussed in this dissertation STA is implemented in Java and relies on a number of auxiliary libraries which aid in data collection \cite{jgit}, storage \cite{mybatis}, and analysis \cite{sax-vsm}. STA also relies on the relational database engine which aids in data indexing and software trajectories construction. 
...
(2) provide adequate phenomena coverage, 
...
\end{tabular}
\end{minipage}%
\caption[An excerpt from pre- and post-release class-characteristic pattern vectors obtained by mining the \textit{Deleted LOC} trajectories in Android OS case study.]{An excerpt from pre- and post-release class-characteristic pattern vectors obtained by mining the \textit{Deleted LOC} trajectories in Android OS case study. The total size of each vector is 622 weighted patterns.}
...
The outlined above procedure was applied to 4 random samples each of which consists of 3 releases (\textit{Train releases} in Table  \ref{android_accuracy}) using three types of software trajectories (\textit{Software metric} in Table \ref{android_accuracy}). The accuracy of resulting classifiers is shown in Table  \ref{android_accuracy}. As shown, the best performing classifier was built using the intervals corresponding to the set of releases \{4,6,9\} (i.e., Android OS API Levels 4, 8, and 11) and the \textit{Deleted LOC} measurements.
...
Table \ref{android_table3} shows details of the classification with the best performing classifier. As shown, it is slightly biased towards post-release. The first 5 class-characteristic patterns for both classes are shown in Table \ref{android_weights}, while examples of software trajectories containing these are shown in Figure \ref{fig:OMAP_patterns}.
...
\end{tabular}
\end{minipage}%
\caption[Accuracy of the LOOCV evaluation for PostgreSQL software trajectories.]{The Leave One Out Cross Validation results for PostgreSQL aggregated software trajectories. The discretization parameters are ordered as the sequence of sliding window size, PAA size, Alphabet size.}
...
For the experimentation I have selected 5 top users whose summary is shown in Table \ref{so_table}. Note, that the top three users were active for the almost whole time span considered in this study.
