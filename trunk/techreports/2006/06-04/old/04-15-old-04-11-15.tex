...
%% Last Modified On: Mon Nov 15 11:45:36 2004
%% RCS: $Id$
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{abstract}
  ...
  reliable software. It can be done before, during or after code
  implementation. Post-coding implementation of unit tests is well
  recognized because people are used to the mindset to validate or verify
  correctness of the existing code. Test-Driven Development (TDD)
  advocates test-first method, in which unit tests are created before code
  implementation based on user stories to drive design of implementation.
  
  TDD is a highly disciplined process. A unit test is created only when
  there is a new requirement or defect in the existing code. In addition
  developers only write enough code to make tests pass. Case studies
  ...
  However, these studies failed to address the discipline issue of TDD and
  how developers executed TDD in their experiments. It is very likely
  developers will do TDD exactly sometimes but write tests after code is
  ready in other cases or there is no unit test created at all in TDD
  experiment. I claim loosely executed TDD will affect experiment
  outcomes and high discipline requirement can be a hurdle to TDD adoption.
  In my thesis I will study how developers write unit tests in development
  in TDD and TDD adoption issue with in-process metrics support from
  Hackystat.
  
  Hacksytat is an automatic in-process metric collection tool. I will
  implement sequential stream analysis on development activities collected
  by Hackystat to study how developers do unit testing in development. A
  four-round experiment in sofware engineering class is being designed to
  study how developer do unit testing in development and verify TDD claims
  on high code quality, development effort saving, 100\% coverage along
  with TDD adoption.
  
  By February 2005 analyses on TDD will be ready for use in experiment for
  subjects to acknowledge their development activities. Experiments will be
  conducted in a spring 2005 software engineering class and data processing
  can be done before September 2005. In fall 2005 I will finish my
  dissertation and prepare for defense in December 2005 or at an early time in
  spring 2006.
