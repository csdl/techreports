%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% -*- Mode: Latex -*- %%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% nsf.tex         : 2009 Smart Consumer Proposal
%% Author          : Philip Johnson
%% Created On      : Tue Mar 31 11:16:58 2009
%% Last Modified By: Philip Johnson
%% Last Modified On: Wed Dec 16 14:07:41 2009
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%   Copyright (C) 2009 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 
 
\documentclass{proposalnsf}
% \usepackage{parskip}


\usepackage[final]{graphicx}

% NSF proposal generation template style file.
% based on latex stylefiles  written by Stefan Llewellyn Smith and
% Sarah Gille, with contributions from other collaborators.

\renewcommand{\refname}{\centerline{References cited}}

% Fix things so that figures tend to stay away from the last page. 
\renewcommand{\topfraction}{0.85}
\renewcommand{\textfraction}{0.1}
\renewcommand{\floatpagefraction}{0.75}

% % this handles hanging indents for publications
% \def\rrr#1\\{\par
% \medskip\hbox{\vbox{\parindent=2em\hsize=6.12in
% \hangindent=4em\hangafter=1#1}}}

% \def\baselinestretch{1}

%% URLs
\usepackage{url}
\usepackage[colorlinks, bookmarks=true]{hyperref}
\usepackage{breakurl}
\urlstyle{same}
\usepackage{floatflt}
\usepackage{wrapfig}

\title{Open Power Quality for the Smart Grid}
\author{Project Proposal to the NSF CyberSEES program 15-524}

\begin{document}

\newpage

%\maketitle
%\pagenumbering{arabic}
%\renewcommand{\thepage} {C--\arabic{page}}
%\renewcommand{\thesection} {C.\arabic{section}}
\setcounter{section}{0}

\input{project-vision}
\input{project-background}
\input{project-plan}
\input{project-prior-results}
\input{project-broader-impacts}

\include{biblio}

\end{document}

