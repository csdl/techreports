...
Mentioned before, recent advances in biological data features mining in Bioinformatics created a wealth of the algorithms and complete software tools. According to \cite{citeulike:964046} there are two categories of approaches for motif finding in biological sequences and the second approach - ``uses patterns with `mismatch representation' which define a signal to be a consensus pattern and allow up to a certain number of mismatches to occur in each instance of the pattern'' - looks very promising in the application for mining motifs from software process artifacts trails. The second group uses patterns with 'mismatch representation'
which define a signal to be a consensus pattern and
